"""(Copied from S-1, table s2.events.) Is a Lakekeeper-vended STS credential scoped to its table, or bucket-wide?

Run inside any container on the v3-s2 network:  python3 - < scope_probe.py
1. Loads s2.events with X-Iceberg-Access-Delegation: vended-credentials (the same path Trino uses).
...
t = get_json(f"{LK}/{prefix}/namespaces/s2/tables/events",
...
def put_delete(name, key):
    code, body = signed("PUT", key, body=b"s2 scope probe")
