...
OIDC_CLIENT_SECRET_SYNC=$(alnum 32)
OIDC_CLIENT_SECRET_JUPYTERHUB=$(alnum 32)
JUPYTERHUB_CRYPT_KEY=$(hex 32)
TRINO_INTERNAL_SECRET=$(hex 32)
LAB_TEST_USER_PASSWORD=$(alnum 16)
OIDC_CLIENT_SECRET_AIRFLOW=$(alnum 32)
OIDC_CLIENT_SECRET_BATCH=$(alnum 32)
AIRFLOW_DB_PASSWORD=$(hex 24)
AIRFLOW_FERNET_KEY=$(openssl rand -base64 32 | tr '+/' '-_')
AIRFLOW_JWT_SECRET=$(hex 32)
OIDC_CLIENT_SECRET_SUPERSET=$(alnum 32)
SUPERSET_SECRET_KEY=$(alnum 40)
SUPERSET_DB_PASSWORD=$(hex 24)
CONSOLE_COOKIE_SECRET=$(alnum 32)
EOT
  )
