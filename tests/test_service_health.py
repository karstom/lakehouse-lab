...
class TestServiceHealth(unittest.TestCase):
    ...
    def test_spark_master_ui(self):
        ...
        try:
            ...
            self.assertEqual(response.status_code, 200, "Spark Master UI should be accessible")
            
            # Check for Spark Master content
            self.assertIn('Spark Master', response.text, "Should contain Spark Master content")
    ...
    def test_airflow_webserver(self):
        ...
        try:
            ...
            self.assertEqual(response.status_code, 200, "Airflow health endpoint should be accessible")
            
            # Test main UI
            response = requests.get(f"{self.base_url}:9020", timeout=10)
            self.assertEqual(response.status_code, 200, "Airflow UI should be accessible")
    ...
    def test_jupyter_lab(self):
        ...
        try:
            ...
            self.assertEqual(response.status_code, 200, "JupyterLab should be accessible with token")
    ...
    def test_superset_dashboard(self):
        ...
        try:
            ...
            self.assertEqual(response.status_code, 200, "Superset health endpoint should be accessible")
            
            # Test main UI
            response = requests.get(f"{self.base_url}:9030", timeout=10)
            self.assertEqual(response.status_code, 200, "Superset UI should be accessible")
    ...
    def test_portainer_ui(self):
        ...
        try:
            ...
            self.assertEqual(response.status_code, 200, "Portainer UI should be accessible")
            
            # Check for Portainer content
            self.assertIn('Portainer', response.text, "Should contain Portainer content")
    ...
    def test_all_services_healthy(self):
        ...
        self.assertEqual(len(failed_services), 0, 
                        f"Some services are not healthy: {failed_services}")
    ...
    def test_docker_containers_running(self):
        ...
        try:
            ...
            expected_containers = [
                ...
                'jupyter', 'airflow', 'superset', 'portainer'
            ]
            ...
            self.assertEqual(len(missing_containers), 0,
                           f"Missing containers: {missing_containers}")
    ...
    def test_service_logs_no_critical_errors(self):
        ...
        critical_error_patterns = [
            'FATAL', 'CRITICAL', 'ERROR: Connection refused',
            'ERROR: No such file or directory', 'OutOfMemoryError',
