...
class TestIcebergIntegration(unittest.TestCase):
    ...
    def test_iceberg_compose_file_exists(self):
        ...
        self.assertTrue(
            self.iceberg_compose.exists(), "docker-compose.iceberg.yml should exist"
        )

    def test_iceberg_documentation_exists(self):
        """Test that Iceberg documentation exists"""
        self.assertTrue(
            self.iceberg_docs.exists(), "ICEBERG.md documentation should exist"
        )
    ...
    def test_iceberg_compose_structure(self):
        ...
        for service in spark_services:
            if service in services:
                ...
                self.assertIn(
                    "volumes", service_config, f"{service} should have volumes"
                )
                ...
                self.assertTrue(
                    iceberg_jar_volume, f"{service} should have iceberg-jars volume"
                )
                    ...
                    for config in spark_iceberg_configs:
                        if config in env_vars:
                            self.assertIsNotNone(
                                env_vars[config], f"{config} should have a value"
                            )
    ...
    def test_iceberg_spark_configuration(self):
        ...
        if "spark-master" in services:
            ...
            if catalog_key in env_vars:
                ...
                self.assertIn(
                    "SparkCatalog", catalog, "Should configure Iceberg Spark catalog"
                )
    ...
    def test_init_script_iceberg_support(self):
        ...
        for check in iceberg_checks:
            self.assertIn(check, content.lower(), f"Init script should contain {check}")
    ...
    def test_iceberg_documentation_structure(self):
        ...
        for section in required_sections:
            self.assertIn(
                section, content, f"Documentation should have {section} section"
            )
    ...
    def test_iceberg_documentation_examples(self):
        ...
        for example in sql_examples:
            self.assertIn(
                example, content, f"Documentation should contain {example} example"
            )
    ...
    def test_iceberg_version_consistency(self):
        ...
        if self.iceberg_compose.exists():
            with open(self.iceberg_compose, "r") as f:
                ...
                version_match = re.search(
                    r"iceberg-spark-runtime.*?(\d+\.\d+\.\d+)", compose_content
                )
                if version_match:
        ...
        if self.init_script.exists():
            with open(self.init_script, "r") as f:
                init_content = f.read()
                version_match = re.search(
                    r"ICEBERG_VERSION.*?(\d+\.\d+\.\d+)", init_content
                )
                if version_match:
        ...
        if self.iceberg_docs.exists():
            with open(self.iceberg_docs, "r") as f:
                doc_content = f.read()
                version_match = re.search(
                    r"Iceberg Version.*?(\d+\.\d+\.\d+)", doc_content
                )
                if version_match:
        ...
        versions = [
            v for v in [compose_version, init_version, doc_version] if v is not None
        ]
        if len(versions) > 1:
            self.assertTrue(
                all(v == versions[0] for v in versions),
...
class TestIcebergFunctionality(unittest.TestCase):
    ...
    def test_spark_iceberg_configuration_validation(self, mock_run):
        ...
        self.assertEqual(
            result.returncode, 0, "Spark with Iceberg should start successfully"
        )
