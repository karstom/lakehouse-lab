...
class TestCrossServiceIntegration(unittest.TestCase):
    ...
    def test_airflow_spark_integration(self):
        ...
        try:
            ...
            self.assertEqual(airflow_response.status_code, 200, "Airflow should be accessible")
            
            # Check Spark Master is accessible
            spark_response = requests.get("http://localhost:8080", timeout=10)
            self.assertEqual(spark_response.status_code, 200, "Spark Master should be accessible")
            
            # Test Spark Master API
            spark_api_response = requests.get("http://localhost:8080/api/v1/applications", timeout=10)
            ...
            try:
                ...
                self.assertIn('lakehouse-test_lakehouse', networks, "Lakehouse network should exist")
                
                # Check services are in the same network
                airflow_inspect = subprocess.run(
                    ['docker', 'inspect', '--format', '{{.NetworkSettings.Networks}}', 'lakehouse-test-airflow-webserver-1'],
                    capture_output=True, text=True
                )
                
                spark_inspect = subprocess.run(
                    ['docker', 'inspect', '--format', '{{.NetworkSettings.Networks}}', 'lakehouse-test-spark-master-1'],
                    capture_output=True, text=True
                )
                ...
                if airflow_inspect.returncode == 0 and spark_inspect.returncode == 0:
                    ...
                    print("⚠️  Network inspection skipped - containers may have different names")
    ...
    def test_superset_duckdb_connection(self):
        ...
        try:
            ...
            self.assertEqual(superset_response.status_code, 200, "Superset should be accessible")
            ...
            # Query the data
            result = conn.execute("SELECT COUNT(*) FROM read_csv_auto('s3://lakehouse/dashboard/sales_data.csv')").fetchone()
    ...
    def test_jupyter_spark_connection(self):
        ...
        try:
            ...
            jupyter_response = requests.get("http://localhost:9040?token=lakehouse", timeout=10)
            self.assertEqual(jupyter_response.status_code, 200, "Jupyter should be accessible")
            ...
            try:
                ...
                result = subprocess.run([
                    'docker', 'exec', 'lakehouse-test-jupyter-1', 
                ...
                print("⚠️  Network connectivity test skipped - docker exec not available")
...
'''
    ...
    def test_minio_spark_s3_access(self):
        ...
        try:
            ...
            minio_response = requests.get("http://localhost:9000/minio/health/live", timeout=10)
            self.assertEqual(minio_response.status_code, 200, "MinIO should be accessible")
            ...
            objects = s3_client.list_objects_v2(Bucket='lakehouse', Prefix='spark-test/')
            uploaded_files = [obj['Key'] for obj in objects.get('Contents', [])]
            self.assertIn('spark-test/test_data.csv', uploaded_files, "Test data should be uploaded")
            
            # Test that data can be read (simulating Spark S3 access)
            downloaded_file = s3_client.get_object(Bucket='lakehouse', Key='spark-test/test_data.csv')
    ...
    def test_service_network_isolation(self):
        ...
        try:
            ...
            self.assertGreater(len(lakehouse_networks), 0, "Lakehouse network should exist")
            ...
            self.assertGreater(len(lakehouse_containers), 5, "Should have multiple lakehouse containers")
    ...
    def test_data_flow_integration(self):
        ...
        try:
            ...
            sales_data = [
                ...
                {'date': '2024-01-03', 'product': 'Widget C', 'sales': 200, 'region': 'East'},
                {'date': '2024-01-04', 'product': 'Widget A', 'sales': 80, 'region': 'West'},
                {'date': '2024-01-05', 'product': 'Widget B', 'sales': 120, 'region': 'North'},
            ]
            ...
            processed_df = pd.DataFrame(agg_result, columns=[
                'product', 'region', 'total_sales', 'transaction_count', 'avg_sales'
            ])
            
            with tempfile.NamedTemporaryFile(mode='w', suffix='.csv', delete=False) as f:
                processed_df.to_csv(f.name, index=False)
                s3_client.upload_file(f.name, 'lakehouse', 'integration/processed_sales.csv')
                os.unlink(f.name)
            ...
            # MinIO access test
            objects = s3_client.list_objects_v2(Bucket='lakehouse', Prefix='integration/')
            integration_files = [obj['Key'] for obj in objects.get('Contents', [])]
            self.assertIn('integration/processed_sales.csv', integration_files, 
            ...
            for host, port in services:
                self.assertTrue(self.wait_for_service(host, port, 5), 
