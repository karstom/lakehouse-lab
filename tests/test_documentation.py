...
class TestDocumentation(unittest.TestCase):
    ...
    def test_iceberg_documentation_structure(self):
        ...
        for section in required_sections:
            self.assertIn(
                section, content,
                f"ICEBERG.md should contain {section} section"
            )
        ...
        for doc_name, doc_path in self.docs.items():
            ...
            for pattern in version_patterns:
                ...
                for match in matches:
                    component = pattern.split('\\s*')[0].replace('[', '').replace(']', '').lower()
    ...
    def test_docker_compose_references(self):
        ...
        for doc_name, doc_path in self.docs.items():
            ...
            for command in compose_commands:
                ...
                if '-f docker-compose.yml' in command and '-f docker-compose.iceberg.yml' in command:
            ...
            for _, code in bash_blocks:
                ...
                for command in commands:
                    ...
                    if command.startswith('rm -rf') and '/' in command:
                        self.fail(
                            f"Dangerous command found in {doc_name}: {command}"
                        )
    ...
    def _extract_code_blocks(self, content: str, language: Optional[str] = None) -> List[Tuple[str, str]]:
    ...
    def _validate_sql_syntax(self, code: str, doc_name: str):
        ...
        sql_keywords = ['SELECT', 'FROM', 'WHERE', 'CREATE', 'TABLE', 'INSERT', 'UPDATE', 'DELETE', 'ALTER', 'DROP']
        ...
        if has_sql_keywords:
            ...
            self.assertEqual(
                paren_count, 0,
                f"Unbalanced parentheses in SQL code in {doc_name}"
            )
    ...
    def _validate_bash_syntax(self, code: str, doc_name: str):
        ...
        for line in lines:
            ...
            if single_quotes % 2 != 0:
                self.fail(f"Unbalanced single quotes in bash code in {doc_name}: {line}")
            
            if double_quotes % 2 != 0:
                self.fail(f"Unbalanced double quotes in bash code in {doc_name}: {line}")
