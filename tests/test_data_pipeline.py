...
class TestDataPipelineEndToEnd(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_data = {
            ...
            'orders': [
                {'order_id': 101, 'customer_id': 1, 'amount': 150.00, 'date': '2024-01-15'},
                {'order_id': 102, 'customer_id': 2, 'amount': 200.00, 'date': '2024-01-16'},
                {'order_id': 103, 'customer_id': 1, 'amount': 75.00, 'date': '2024-01-17'}
            ]
        }
    ...
    def test_csv_ingestion_to_minio(self):
        ...
        try:
            ...
            self.assertIn('data/customers.csv', uploaded_files, "Customers CSV should be uploaded")
            self.assertIn('data/orders.csv', uploaded_files, "Orders CSV should be uploaded")
    ...
    def test_duckdb_s3_integration(self):
        ...
        try:
            ...
            # Read data from S3
            customers_query = "SELECT * FROM read_csv_auto('s3://lakehouse/data/customers.csv')"
            customers_result = conn.execute(customers_query).fetchall()
            
            orders_query = "SELECT * FROM read_csv_auto('s3://lakehouse/data/orders.csv')"
            ...
            self.assertGreater(len(analytical_result), 0, "Should get analytical results")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if sample_dag:
                    ...
                    # Trigger DAG run
                    trigger_url = f"http://localhost:9020/api/v1/dags/{sample_dag}/dagRuns"
                    trigger_data = {
                        "conf": {},
                    ...
                    if trigger_response.status_code in [200, 201]:
                        ...
                        time.sleep(5)
                        dag_runs_url = f"http://localhost:9020/api/v1/dags/{sample_dag}/dagRuns"
                        runs_response = requests.get(dag_runs_url, auth=('admin', 'admin'), timeout=10)
            ...
            self.assertEqual(basic_response.status_code, 200, "Airflow should be accessible")
...
'''
        ...
        try:
            ...
            for check in quality_checks:
                ...
                if result == check['expected']:
                    ...
                    print(f"❌ {check['name']}: FAILED (expected {check['expected']}, got {result})")
            
            conn.close()
            
            # Assert all checks passed
            self.assertEqual(len(failed_checks), 0, f"Data quality checks failed: {failed_checks}")
            
            print(f"✅ Data quality checks completed: {passed_checks}/{len(quality_checks)} passed")
    ...
    def test_end_to_end_pipeline(self):
        ...
        try:
            ...
            summary_df = pd.DataFrame(summary_result, columns=[
                'city', 'customer_count', 'total_orders', 'total_revenue', 'avg_order_value'
            ])
            ...
            self.assertIn('reports/city_summary.csv', report_files, "Summary report should be created")
    ...
    def test_performance_baseline(self):
        ...
        try:
            ...
            print(f"✅ Performance baseline: Query completed in {query_time:.2f} seconds")
